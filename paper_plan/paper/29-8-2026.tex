...
completed 500 calls across DeepSeek V4 Flash 0731 and Qwen3 8B, but its paired
diagnostic intervals did not establish a grounding or model-fidelity advantage.
...
\begin{figure*}[t]
    ...
    \begin{tikzpicture}[font=\footnotesize, node distance=5mm and 7mm]
        ...
        \node[deedybox, minimum width=34mm, below=13mm of scenario] (core)
            {MiroFish/MarketFish core\\world, actors, simulation};
        ...
        \node[draw=deedyorange, rounded corners=4pt,
            ...
            label={[text=deedyorange,font=\bfseries\footnotesize]above:Simulation path}] {};
...
simulation layer adapted from the MarketFish component design
~\cite{marketfish2026architecture}. This layer
...
adapts the MarketFish architecture and workflow described by the ISAL-NLP
manuscript~\cite{marketfish2026architecture}; DEEDY adds the scenario packet,
GraphRAG grounding, Thai actor attributes, provenance, and the external
holdout protocol described in this paper.
...
\begin{figure*}[t]
    ...
    \caption{Core-technology architecture adapted from MarketFish
    ~\cite{marketfish2026architecture}. The orchestrator combines configurable
    exposure and intervention with a hybrid cognitive--stochastic population;
    agents use short- and long-term memory, and their traces feed reporting and
    the live application. Solid arrows denote the main execution/data path;
    dashed arrows denote memory or configuration access.}
...
therefore integrates the component paper's hybrid ABM design
...
Experiments~1--3. Zep-dependent graph-memory operations were not required for
this bounded prompt-level pilot.
...
calls and 750 expanded metric rows. Input SHA-256 hashes, parameters, run-level
...
\texttt{llm\_exp123\_manifest.json}, and the accompanying machine-readable
...
Its self-labeled prompt diagnostics do not satisfy the pre-registered
...
frontend; 1,000 non-generative runs and 500 OpenRouter calls completed with
run-level provenance. DeepSeek V4 Flash 0731 cost less in this run and Qwen3 8B
returned all requested items faster, but paired sentiment-JSD intervals did not
establish a fidelity advantage for grounding, either model, or the LLM tier.
...
distribution; it therefore demonstrates pipeline sensitivity, not campaign
effectiveness.
