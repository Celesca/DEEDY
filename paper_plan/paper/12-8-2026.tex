...
~\cite{arreerard2022survey}, but cultural capability remains a distinct
evaluation target. Second, social-data collection is platform-dependent and
incomplete. A reproducible study must record the source post, collection time,
requested limit, returned count, and visibility constraints.

A further methodological problem is how to evaluate the simulation. A
generated comment may read naturally and resemble the writing style of a real
social-media user while still expressing the wrong stance, missing important
consumer concerns, or producing an unrealistic balance of positive and
negative reactions. Thus, realistic language alone does not show that the
simulation reproduces consumer behavior~\cite{argyle2023out,
mooney2025coherent,schwager2026conditioned}. DEEDY therefore keeps the real
campaign comments hidden from the agents and uses them only as a held-out
reference for evaluation. It also repeats each simulation under multiple
random seeds, because one convincing result may be unstable or accidental.
...
Thai online language remains challenging for downstream semantic analysis.
Informal spelling, repeated characters, code-mixing, and culturally specific
references can change the intended meaning, so cultural competence is a
separate evaluation target from general language performance
~\cite{arreerard2022survey}. Campaign meaning is also contextual: sarcasm can
reverse literal sentiment, and conversation history can improve its
interpretation~\cite{hazarika2018cascade}. These observations motivate the
separation of sentiment, campaign stance, narrative, and sarcasm in evaluation.
...
DEEDY collects two distinct forms of evidence. First, campaign-construction
data include the campaign brief, creative message, product information, and
dated public reporting. They describe the situation given to the simulated agents.
Second, real consumer comments are collected from selected public Facebook,
Instagram, and TikTok posts through Apify, with a requested limit of 1,000
comments for each post. These collected comments are the \emph{observed
reference data}; they are not assumed to represent every Thai consumer or
every comment visible on a platform.

Realistic-looking generated comments do not necessarily mean that the
simulation reproduces real consumer behavior. Therefore, the campaign
information used to construct the simulated environment is kept separate from
the real comments used for evaluation. DEEDY reserves those comments as a
held-out reference set and does not show them to the simulated agents. It then
compares simulated and observed sentiment, stance, narratives, and interaction
patterns across multiple random runs, because one convincing run may be
unstable or accidental.

The collected comments and their human-verified labels serve different roles.
The original comments are the observed reactions that the simulation is
expected to approximate. Native Thai annotators verify labels such as
sentiment, campaign stance, narrative, and sarcasm so that these characteristics
can be scored consistently. The following stages describe how campaign posts
are selected, how their comments are collected and preserved, and how the two
evidence streams are separated before simulation and evaluation.
...
Campaign facts used to construct the simulated scenario remain separate from
consumer reactions. This distinction supports both retrospective campaign
analysis and a stricter prospective design in which only information available
at campaign launch is provided to the agents.
...
turn~\cite{hazarika2018cascade}. The language model is configurable through an
OpenAI-compatible endpoint, and every run records its model identifier and
decoding settings.
