...
fidelity nor a message-clarity advantage.
...
Figure~\ref{fig:judge-workflow} adds the executed LLM-as-a-judge path. The raw
crawl in \texttt{social\_comments\_crawled.jsonl} supplies authentic text,
whereas \texttt{campaign\_sentiment\_analysis.jsonl} and
\texttt{campaign\_sentiment\_summary.csv} remain post-judging references. The
real text passes through identifier and contact-pattern redaction; the
simulation log contributes one deterministically sampled visible reaction per
eligible generation call. Same-campaign pairs are blinded, judged in both A/B
orders, mapped back to real or synthetic identity only after scoring, and
aggregated at the pair level. Human review remains a separate validation path
rather than being replaced by the automated judge.

\begin{figure*}[t]
    \centering
    \resizebox{0.98\textwidth}{!}{%
    \begin{tikzpicture}[font=\footnotesize, node distance=7mm and 8mm]
        \node[deedybox, minimum width=29mm] (real)
            {Authentic comments\\nested crawl records};
        \node[deedybox, minimum width=29mm, below=of real] (synthetic)
            {Experiment 4--5 logs\\visible agent reactions};

        \node[deedybox, minimum width=29mm, right=of real] (sanitize)
            {Text sanitization\\remove direct identifiers};
        \node[deedybox, minimum width=29mm, right=of synthetic] (sample)
            {Deterministic sampler\\exclude all-silence calls};

        \node[deedybox, minimum width=29mm, right=11mm of sanitize,
            yshift=-8mm] (pair)
            {Same-campaign pair\\source hidden};
        \node[deedybox, minimum width=27mm, right=of pair] (blind)
            {Random A/B order\\then reversed order};
        \node[deedybox, minimum width=29mm, right=of blind] (judge)
            {GLM-5.3-Flash judge\\scores and preference};
        \node[deedybox, minimum width=29mm, right=of judge] (aggregate)
            {Pair-level aggregation\\bias and uncertainty};
        \node[deedybox, minimum width=29mm, below=of aggregate] (human)
            {Native-Thai review\\pending validation};

        \draw[deedyarrow] (real) -- (sanitize);
        \draw[deedyarrow] (synthetic) -- (sample);
        \draw[deedyarrow] (sanitize) -- (pair);
        \draw[deedyarrow] (sample) -- (pair);
        \draw[deedyarrow] (pair) -- (blind);
        \draw[deedyarrow] (blind) -- (judge);
        \draw[deedyarrow] (judge) -- (aggregate);
        \draw[deedydashed] (aggregate) -- (human);

        \node[draw=deedyred, rounded corners=4pt,
            fit=(real)(sanitize), inner sep=5pt,
            label={[text=deedyred,font=\bfseries\footnotesize]above:Held-out real path}] {};
        \node[draw=deedyorange, rounded corners=4pt,
            fit=(synthetic)(sample), inner sep=5pt,
            label={[text=deedyorange,font=\bfseries\footnotesize]below:Simulation path}] {};
        \node[draw=deedyblue, rounded corners=4pt,
            fit=(pair)(blind)(judge)(aggregate)(human), inner sep=5pt,
            label={[text=deedyblue,font=\bfseries\footnotesize]above:Blinded evaluation}] {};
    \end{tikzpicture}%
    }
    \caption{Executed LLM-as-a-judge workflow. Only sanitized comment text and
    frozen scenario facts enter the judge prompt. Source identity, authors,
    URLs, aggregate sentiment targets, and campaign-level analyses are hidden.
    Each pair is judged twice with reversed order; the dashed arrow marks the
    native-Thai validation that remains pending.}
    \label{fig:judge-workflow}
\end{figure*}
...
\subsubsection{Executed GLM-5.3-Flash Judge Protocol}

The secondary judge evaluation uses \texttt{z-ai/glm-5.3-flash} through
OpenRouter~\cite{openrouter2026glm53flash}. The generator and judge roles are
separated: DeepSeek V4 Flash 0731 and Qwen3 8B produce the Experiment~4--5
reactions, while GLM-5.3-Flash sees only the frozen scenario facts and two
source-blinded candidate comments. From every generation call with at least one
visible action, one schema-valid non-empty reaction is selected by a stable
hash. A distinct authentic record from the same campaign is sampled without
replacement from the 5,000 nested comments in
\texttt{social\_comments\_crawled.jsonl}. Fifty-nine all-silence calls contain
no text to compare and remain in the action-rate analysis but are excluded from
this text-level evaluation.

Before transmission, URLs, e-mail addresses, handles, and phone-like strings
are replaced with typed placeholders; author objects, comment IDs, post URLs,
platform labels, and source identity are omitted. Each of the 741 eligible
pairs is evaluated twice, once in a deterministic randomized A/B order and once
with the order reversed. The judge independently assigns integer scores from 1
to 5 for Thai naturalness, campaign relevance, social-media plausibility,
pragmatic/cultural fit, contextual specificity, and unsupported-claim risk
(where 5 denotes greater risk). It also assigns three-way sentiment, stance,
and an A/B/tie authenticity preference. A returned \texttt{mixed} sentiment is
mapped to neutral to match the supplied three-class reference. Only source
hashes, structured scores, mapped preferences, provider metadata, and a short
non-quoting evidence summary are persisted.

The pair, rather than either orientation, is the unit of analysis. We report
real-, synthetic-, and tie-preference shares; a simulated-realism score defined
as synthetic preference plus half the tie share; reversed-order preference
consistency; raw A-choice share among non-ties; source-specific mean scores;
and synthetic-minus-real paired gaps with deterministic 2,000-resample
bootstrap intervals. Judge sentiment distributions are compared by JSD with
the post-judging aggregate reference in
\texttt{campaign\_sentiment\_summary.csv};
\texttt{campaign\_sentiment\_analysis.jsonl} is used only for provenance and
topic consistency. These two reference files are never included in a judge
prompt. Because GLM is a single automated evaluator, these results remain
secondary to the native-Thai human protocol above.
...
The judge run fixes GLM temperature at 0, uses stable request seeds and JSON
output, and executes the two orientations of each eligible pair. The primary
pass used a 2,200-token completion ceiling and concurrency 40; bounded repairs
raised the ceiling to 3,200 for ten incomplete responses without changing the
sample or rubric. The provider required internal reasoning, but the runner
parsed and retained only the final structured judgment. All failed or malformed
calls were retried by stable call key, and completed calls were resumed rather
than regenerated.
...
\subsection{Automated Pilot Results}
\label{sec:pilot-results}

\subsubsection{Execution Scope and Reproducibility}
...
operations were not required for these bounded prompt-level pilots. On 31
August, GLM-5.3-Flash completed a separate blinded LLM-as-a-judge evaluation.
...
Of the 800 generation calls, 741 contained at least one visible, non-empty
reaction. These were paired with 741 same-campaign records sampled from the
5,000 authentic comments and judged in both A/B orders, yielding 1,482 valid
judge calls. Fifty-nine all-silence calls remained in action-rate metrics but
were excluded from text comparison.
...
\texttt{llm\_judge\_exp45\_manifest.json}, with the accompanying
machine-readable files. Authentic comments were absent from all generation
prompts. The judge received sanitized comment text only: author objects, IDs,
URLs, platform labels, and source identity were withheld, and no raw comment
text was persisted in the judge result bundle.
...
panel were run with ten seeds. Eligible generated comments were evaluated by
the blinded, reversed-order GLM judge; a full LLM/OASIS reply trajectory remains pending. \\
Exp. 5: Application & Docker health, result provenance, a paired internal ABM
counterfactual, and a paired two-model LLM clarity-frame panel were run. The
baseline and clarity comments entered the judge diagnostic; analyst usefulness,
human quality, and presentation effects were not rated. \\
\bottomrule
\end{tabular}
\end{table*}

\subsubsection{Dataset and Trace Audit}
...
\subsubsection{OpenRouter LLM Diagnostics: Experiments 1--3}
...
\subsubsection{Social-Mechanism Sensitivity}
...
\subsubsection{Leakage-Guarded LLM Cognitive-Agent Panel: Experiments 4--5}
...
\subsubsection{Blinded GLM-5.3-Flash Judge Results}

All 1,482 GLM-5.3-Flash calls completed with schema-valid outputs, covering 741
real--synthetic pairs and two reversed orientations per pair. Across the 1,482
judgments, the authentic comment was preferred as more likely real in 0.5931,
the synthetic reaction in 0.4055, and a tie in 0.0013. The resulting simulated-
realism score was 0.4062 (bootstrap 95\% CI 0.3742--0.4379), below the 0.5
indifference point. Mapped preference agreed across the two orientations for
0.8354 of pairs. However, among non-ties the raw A candidate was selected in
0.5588 of judgments, demonstrating measurable position sensitivity despite
blinding and motivating pair reversal rather than a single presentation.

Table~\ref{tab:judge-global-scores} shows a different aspect of quality.
Synthetic reactions received higher mean Thai-naturalness, campaign-relevance,
pragmatic/cultural-fit, and contextual-specificity scores, with intervals that
exclude zero. Social-media plausibility differed by only 0.0445 and its interval
included zero; unsupported-claim risk was also indistinguishable. Thus, GLM
often rated the generated response as polished, explicit, and on-topic while
still recognizing the shorter or less curated authentic comment as more likely
real. Campaign relevance is especially susceptible to this asymmetry because
real social threads contain fragments, tagging, jokes, and side conversations,
whereas the synthetic prompt explicitly requests a campaign reaction.

\begin{table*}[t]
\caption{Global GLM-5.3-Flash scores over 741 reversed-order pairs. Scores are
1--5; for unsupported-claim risk, higher is worse. Gaps are synthetic minus
real with deterministic 2,000-resample bootstrap intervals.}
\label{tab:judge-global-scores}
\centering
\footnotesize
\begin{tabular}{@{}lrrrr@{}}
\toprule
Dimension & Real mean & Synthetic mean & Gap & 95\% CI \\
\midrule
...
The judge-labeled sentiment distribution for the sampled authentic comments
had JSD 0.0769 from the supplied aggregate reference, compared with 0.2151 for
the synthetic reactions. This independently re-identifies the sentiment-
correspondence weakness seen in the generator self-labels, although the
reference itself remains a weak aggregate analysis rather than human gold.
Reversed-order sentiment labels agreed for 0.8920 of real comments and 0.9244
of synthetic comments.

Table~\ref{tab:judge-model-summary} reports model-level slices. DeepSeek's
simulated-realism score exceeded Qwen's in both experiments, but neither model
can be declared superior from one automated judge. The Qwen slices also exclude
59 all-silence calls (40 in Experiment~4 and 19 in Experiment~5), producing
unequal text-eligible samples and selection toward agents that chose a visible
action. At campaign level, KFC Bucket Ware was the only campaign for which the
synthetic reaction was preferred more often than the real comment (realism
0.5106, CI 0.4401--0.5845). Top Charoen showed the largest authenticity gap
(realism 0.2468, CI 0.1823--0.3129). These differences are diagnostics of the
current packets and generators, not campaign-effect estimates.

\begin{table*}[t]
\caption{GLM judge summary by experiment and generator. The realism score is
synthetic preference plus half the tie share. Thai and plausibility gaps are
synthetic minus real.}
\label{tab:judge-model-summary}
\centering
\footnotesize
\begin{tabular}{@{}llrrrrrr@{}}
\toprule
Experiment & Generator & Pairs & Real pref. & Synth. pref. & Realism (95\% CI) & Thai gap & Plaus. gap \\
\midrule
...
For Experiment~5, the descriptive realism score changed from 0.4500 to 0.4700
under DeepSeek's clarity frame and from 0.4333 to 0.2847 under Qwen's. These
rows use different authentic comparison samples and contain 50/50 versus 45/36
eligible baseline/clarity pairs, respectively; they therefore cannot support a
paired clarity-effect claim. The complete judge run cost \$0.5580 and averaged
30.23 seconds per orientation. Together with the \$0.1386 Experiment~1--3 and
\$0.3586 Experiment~4--5 generation runs, recorded OpenRouter cost was
\$1.0552. Full pair-, campaign-, model-, and condition-level summaries are
stored with the judge manifest.

\subsubsection{Conditional Message-Frame Smoke Test}
...
across all five experiment groups. Its self-labeled prompt diagnostics and
single-model judge do not satisfy the pre-registered
...
The judge adds information that aggregate sentiment JSD cannot provide, but it
does not simply make the system look better. GLM preferred authentic comments
more often and placed the synthetic realism interval below 0.5, while assigning
the synthetic reactions higher naturalness, relevance, cultural-fit, and
specificity scores. This combination is consistent with a ``too polished and
too relevant'' failure mode: prompted agents produce coherent standalone
responses, whereas real threads include terse fragments, social tagging,
off-topic exchanges, repetition, and locally meaningful shorthand. Optimizing
only the six rubric scores could therefore reduce authenticity by removing the
very irregularities that distinguish observed discourse.

The judge result also sharpens the interpretation of Experiments~4--5.
Experiment~4's feed and network prompts condition independent cognitive panels;
they are not observed exposure paths. Consequently, differences in judge
realism across those rows measure how prompt conditions change generated text,
not whether a platform recommender or network topology was reconstructed.
Experiment~5's baseline and clarity rows likewise lack observed outcomes for
both treatments. The judge can assess response quality under each hypothetical
frame, but it cannot turn the clarity contrast into a causal campaign estimate.

Campaign variation indicates where additional scenario and persona work may be
most useful. KFC's value-and-redemption packet produced synthetic reactions that
were at least competitive with sampled real comments under this judge, whereas
Top Charoen's activity-and-redemption mechanism remained readily distinguishable.
This could reflect missing eligibility, fulfilment, or conversational context;
it could also reflect campaign-specific judge priors. The result motivates
error analysis by native-Thai raters rather than campaign ranking by GLM alone.

Finally, the 0.5588 A-choice rate and 0.8354 reversal consistency show that
blinding is necessary but insufficient. A different model, provider, rubric,
or decoding route could change both absolute scores and pairwise preference.
The present judge is useful for scalable screening and for locating failure
modes; acceptance still requires a pre-registered native-Thai sample, judge--
human agreement, and adjudication of disagreements.
...
The executed judge transmitted sanitized authentic comment text to an external
OpenRouter-served model. Direct identifiers, URLs, handles, e-mail addresses,
and phone-like strings were removed and raw text was not stored in judge
artifacts, but sanitization cannot guarantee that every indirect identifier or
sensitive disclosure is removed. Future use should confirm provider retention
settings, data-processing terms, platform permissions, and institutional ethics
approval before transmission; a locally hosted judge is preferable when those
...
run-level provenance, followed by 1,482 GLM judge calls over 741 blinded pairs.
DeepSeek V4 Flash 0731 cost less in the Experiment~1--3
...
The independent judge preferred authentic comments more often despite scoring
synthetic reactions as more polished and campaign-relevant; it also exposed
position sensitivity and substantial campaign variation. Together, these paths
demonstrate pipeline sensitivity and auditability, not campaign effectiveness.
...
ratings, including judge--human agreement on a pre-registered subset. The
resulting experiments should retain the manifest and privacy
