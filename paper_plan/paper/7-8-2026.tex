...
Facebook comments are collected in a separate, authenticated Crawl4AI pass
using the manually refreshed \texttt{facebook\_2} browser profile. Discovery
starts from Thai news and community posts returned by expressions such as
\texttt{PARAMETER gelato} and their Thai-language equivalents, rather than
treating the brand's official page as
the only source. The crawler opens each post, expands visible comment and reply
controls, and scrolls until the profile-visible thread is exhausted. Each
comment is written to an immutable UTF-8 JSONL record with a deterministic
comment identifier, canonical post URL, source/page key, text, timestamp when
shown, locale, and collection time. This is a reproducible observation of the
authenticated profile's view, not a claim that Facebook exposes every comment
or a complete platform census.

\subsubsection{News-grounded social-comment benchmark construction}
\label{sec:comment-benchmark}

The benchmark unit is an observed comment attached to a concrete post about the
target event. To construct it, the pipeline (i) freezes the Thai query set and
retrieval date, (ii) discovers news/community post permalinks, (iii) collects
the profile-visible comments and replies with Crawl4AI, (iv) de-duplicates by
comment and canonical-post identifiers, and (v) freezes the raw JSONL before
any language-model annotation. News reports and official statements provide
event context for the scenario set $S_e$; the resulting Facebook reactions are
stored as a separate held-out set $G_e^{\mathrm{FB}}$. For the PARAMETER gelato
case, the 7 August 2026 snapshot contains 219 unique comments from seven
posts. Five posts have at least 20 comments (205 comments in total); the
high-volume threads are from The Standard Wealth, Bangkok Biz News, CheckBait,
Thansettakij, and Prachachat Online. A lower-volume Thairath Money thread and
one official PARAMETER Thailand thread are retained as provenance-stratified
controls. The official thread is therefore not the sole source of the
benchmark.

$G_e^{\mathrm{FB}}$ is an operational grounded-truth reference for evaluating
simulated reactions, not human gold labels and not a census of Facebook users.
No source, quoted passage, comment identifier, or near-duplicate is allowed in
both $S_e$ and $G_e^{\mathrm{FB}}$. The manifest records the query expressions,
authenticated profile and visibility scope, canonical URLs, retrieval
timestamps, content checksums, de-duplication version, source stratum, and
split assignment. Freezing this manifest before annotation makes the dataset
re-runnable and permits later Thai human adjudication without changing the
observed target set.

\begin{table}[t]
\caption{Data-pipeline snapshot inspected on 7 August 2026. Counts describe
...
\begin{table}[t]
\caption{PARAMETER Facebook benchmark snapshot on 7 August 2026. Counts are
comments visible to the authenticated \texttt{facebook\_2} Crawl4AI profile;
sentiment, stance, and sarcasm values are LLM weak annotations.}
\label{tab:parameter-comments}
\centering
\footnotesize
\begin{tabular}{@{}lr@{}}
\toprule
Artifact & Count \\
\midrule
Unique comments / source posts & 219 / 7 \\
High-engagement posts (at least 20 comments) & 5 \\
Comments in high-engagement posts & 205 \\
Model annotations / failed batches & 219 / 0 \\
Negative / neutral / positive sentiment & 101 / 86 / 32 \\
Critical / inquiry / neutral stance & 98 / 31 / 58 \\
Sarcasm or irony detected & 42 (19.2\%) \\
\bottomrule
\end{tabular}
\end{table}
...
\texttt{llm\_comment\_analysis.py} sends the frozen Facebook records to an
OpenAI-compatible large language model in small batches and requires one
structured JSON annotation for every deterministic comment identifier. The
annotation schema requests sentiment and score, event stance, emotions,
sarcasm/irony, a semantic cluster, themes, confidence, and a short rationale.
Clusters are selected from a fixed taxonomy covering price/value, taste/quality,
safety, consumption rules, brand communication, ownership/business, service
experience, comparison, and general reaction. A second synthesis call produces
an executive summary, positive and negative drivers, polarization axes,
sarcasm patterns, recommendations, and caveats. The PARAMETER run used
\texttt{qwen/qwen3.7-flash}: all 219 comments received annotations and no batch
failed. The observed distributions are negative/neutral/positive sentiment
101/86/32, critical/inquiry/neutral stance 98/31/58, and 42 comments (19.2\%)
with sarcasm or irony. The most frequent clusters were brand communication
(68), general reaction (62), and ownership/business (40).

These outputs are \emph{weak pre-annotations}, not ground truth. The summary
derived from $S_e$ may enter MiroFish, while $G_e^{\mathrm{FB}}$ remains
outside actor memory and its labels are used only for scoring and error
analysis. Before the main experiment, native Thai annotators will review a
frozen sample, adjudicate disagreements, report Krippendorff's $\alpha$, and
audit sarcasm and source-stratum errors. Separating the news/context summary
from the Facebook reaction labels prevents validation reactions from leaking
into actor memory.
...
For the news-grounded Facebook benchmark, the query date, source strata, and
post-level split are frozen before requesting any LLM labels. The threshold of
20 comments is descriptive for identifying high-engagement threads and is not
optimized against an evaluation score. All seven PARAMETER posts remain in the
manifest so that future reruns can test source-balanced sampling, temporal
drift, and sensitivity to the authenticated profile's visibility.
