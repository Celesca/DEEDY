...
class SimulationRunState:
    ...
    # 泰国社会平台运行状态
    thai_society_running: bool = False
    thai_society_current_round: int = 0
    thai_society_simulated_hours: float = 0.0
    thai_society_completed: bool = False
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        elif platform == "thai_society":
            script_name = "run_thai_society_simulation.py"
            state.thai_society_running = True
        else:
        ...
        if not os.path.exists(script_path):
            ...
            state.thai_society_running = False
        ...
        try:
            ...
            #   thai_society_actions.jsonl - Thai Society 动作日志
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        ...
        thai_society_actions_log = os.path.join(sim_dir, "thai_society_actions.jsonl")
        ...
        try:
            while process.poll() is None:  # 进程仍在运行
                ...
                # 读取 Thai Society 动作日志
                if os.path.exists(thai_society_actions_log):
                    thai_society_position = cls._read_action_log(
                        thai_society_actions_log, thai_society_position, state, "thai_society"
                    )
            ...
            if os.path.exists(thai_society_actions_log):
                cls._read_action_log(thai_society_actions_log, thai_society_position, state, "thai_society")
    ...
    def _read_action_log(
        ...
        Args:
            ...
            position: 上次读取位置
            state: 运行状态对象
            platform: 平台名称 (twitter/reddit/thai_society)
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    if platform == "twitter":
                                        ...
                                        state.thai_society_completed = True
                                        state.thai_society_running = False
                                ...
                                # 更新轮次信息（从 round_end 事件）
                                    ...
                                    if platform == "twitter":
                                        ...
                                        if round_num > state.thai_society_current_round:
                                            state.thai_society_current_round = round_num
                                        state.thai_society_simulated_hours = simulated_hours
                                    
                                    # 总体轮次取各平台的最大值
                                    state.current_round = max(
                                        state.twitter_current_round, 
                                        state.reddit_current_round,
                                        state.thai_society_current_round
                                    )
                                    # 总体时间取各平台的最大值
                                    state.simulated_hours = max(
                                        state.twitter_simulated_hours, 
                                        state.reddit_simulated_hours,
                                        state.thai_society_simulated_hours
                                    )
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        thai_society_log = os.path.join(sim_dir, "thai_society_actions.jsonl")
        
        # 检查哪些平台被启用（通过文件是否存在判断）
        twitter_enabled = os.path.exists(twitter_log)
        reddit_enabled = os.path.exists(reddit_log)
        thai_society_enabled = os.path.exists(thai_society_log)
        ...
        if reddit_enabled and not state.reddit_completed:
            return False
        if thai_society_enabled and not state.thai_society_completed:
            return False
        
        # 至少有一个平台被启用且已完成
        return twitter_enabled or reddit_enabled or thai_society_enabled
