...
TOOL_DESC_TRACE_STATISTICS = """\
【确定性轨迹统计 - 事实溯源与宏观指标】
直接统计模拟产生的动作日志（actions.jsonl），返回可直接引用的确定性数字。
所有数值均为直接计数，不含任何LLM推断，因此不会产生幻觉。

【返回内容】
1. 扩散规模：动作总数、分平台动作数、参与Agent数、人均动作数
2. 动作类型构成：CREATE_POST / REPOST / LIKE_POST / FOLLOW 等各自的次数与占比
3. 立场分布（公开表达 vs 私下意见）：按动作数统计的 supportive / opposing / neutral 占比
4. Preference Falsification Gap（偏好伪装鸿沟）：公开表达与私下真实意见的差异，揭示沉默的大多数（Silent Dissent / Silent Support）
5. 时间线：每一轮的动作数及峰值轮次
6. 最活跃Agent Top 10（含其立场）

【使用场景】
- 报告中需要引用确凿的宏观数据
- 描述传播规模、参与度、爆发时点
- 必须用于回答关于“两层意见鸿沟”（公开表达 vs 私下真实想法）的问题

【重要限制】
本工具不提供情感极性（sentiment）数值——动作日志中不存在该字段。
若模拟尚未运行或日志已清理，工具会明确返回“不可用”，
此时禁止自行推测任何扩散或立场数字。"""
...
7. 【Audit Trail - 强制引用来源与深层心理分析】
   - 每次引用或提及特定Agent的言行时，必须在文末或引用后标注来源 (Audit Trail)，例如: `[来源: actions.jsonl - Agent_001]` 
   - 任何涉及特定人群规模、立场的数据结论，必须在报告内指明是由 `trace_statistics` 工具所提供。
   - 强烈建议对比Agent的「公开表达」与「私下真实想法」(Private Stance)，揭示其背后的恐惧或社会压力。
...
> "微博贡献了68%的首发声量..." [来源: actions.jsonl - Agent_042]
...
1. 内容必须基于工具检索到的模拟数据，并且在描述宏观趋势时必须调用 trace_statistics 获取确定性的扩散规模和立场分布数据！
2. 大量引用原文来展示模拟效果
...
4. 【引用格式规范 - 必须单独成段】
   ...
   > "校方的应对模式在瞬息万变的社交媒体环境中显得僵化和迟缓。" [来源: actions.jsonl - Agent_103]
...
class ReportAgent:
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "trace_statistics": {
                ...
                "description": TOOL_DESC_TRACE_STATISTICS,
                "parameters": {}
            }
        }
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            ...
            elif tool_name == "trace_statistics":
                # 确定性轨迹统计 - 直接读取动作日志，不经过LLM
                return self._calculate_trace_statistics()
    ...
    def _load_agent_stance_map(self, sim_dir: str) -> Dict[int, str]:
        """从 simulation_config.json 读取 agent_id -> stance 映射

        立场由配置生成阶段确定并落盘，因此是确定性的，
        不需要再让LLM对动作内容做二次判断。
        """
        ...
        try:
            ...
            logger.warning(f"读取 simulation_config.json 失败: {e}")
            return {}
        ...
        for cfg in config.get("agent_configs", []):
            ...
            if agent_id is not None:
                stance_map[agent_id] = cfg.get("stance", "neutral")
        return stance_map

    def _calculate_trace_statistics(self) -> str:
        """计算确定性轨迹统计（扩散规模 / 动作构成 / 立场分布 / 时间线）

        数据源为各平台的 actions.jsonl（由 ActionLogger 落盘的真实动作轨迹），
        立场来自 simulation_config.json。全部为直接统计，不含任何LLM推断。
        """
        from ..config import Config
        sim_dir = os.path.join(Config.RUN_STATE_DIR, self.simulation_id)

        stance_map = self._load_agent_stance_map(sim_dir)

        platforms = ["twitter", "reddit", "thai_society"]
        platform_counts = {p: 0 for p in platforms}
        action_type_counts: Dict[str, int] = {}
        stance_counts: Dict[str, int] = {}
        private_stance_counts: Dict[str, int] = {}
        falsification_gap: Dict[str, int] = {"silent_dissent": 0, "vocal_dissent": 0, "silent_support": 0, "vocal_support": 0}
        round_counts: Dict[int, int] = {}
        participants = set()
        agent_action_counts: Dict[int, int] = {}
        ...
        for platform in platforms:
            ...
            with open(actions_file, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    # round_start / round_end / simulation_start 等事件行没有 agent_id，跳过
                    ...
                    # Phase 8.1 & 8.2: Falsification Gap (Private vs Public)
                    action_args = entry.get("action_args", {})
                    private_stance = action_args.get("private_stance", "unknown")
                    private_stance_counts[private_stance] = private_stance_counts.get(private_stance, 0) + 1

                    if private_stance == "opposing":
                        if action_type in ["SILENT_SHIFT", "NO_ACTION"]:
                            falsification_gap["silent_dissent"] += 1
                        else:
                            falsification_gap["vocal_dissent"] += 1
                    elif private_stance == "supportive":
                        if action_type in ["SILENT_SHIFT", "NO_ACTION"]:
                            falsification_gap["silent_support"] += 1
                        else:
                            falsification_gap["vocal_support"] += 1

                    # Ignore NO_ACTION for active interaction metrics
                    ...
                    agent_action_counts[agent_id] = agent_action_counts.get(agent_id, 0) + 1
                    if entry.get("agent_name"):
                        agent_names[agent_id] = entry["agent_name"]
                    ...
                    stance = stance_map.get(agent_id, "unknown")
                    stance_counts[stance] = stance_counts.get(stance, 0) + 1

        if not found_any_log:
            return (
                "【轨迹统计不可用】未找到任何动作日志（actions.jsonl）。\n"
                "该模拟可能尚未运行、正在运行中，或运行数据已被清理。\n"
                "请勿据此推测任何扩散规模或立场分布数据。"
            )

        if total_actions == 0:
            return "【轨迹统计】已找到动作日志，但其中不含任何Agent动作记录（总动作数为 0）。"

        def _pct(n: int) -> str:
            return f"{n / total_actions * 100:.1f}%"

        lines = ["=== 确定性轨迹统计（直接统计自动作日志，非LLM推断）==="]

        # 1. 扩散规模
        lines.append("")
        lines.append("【扩散规模】")
        lines.append(f"- 动作总数：{total_actions}")
        for platform in platforms:
            if platform_counts[platform]:
                lines.append(
                    f"- {platform.capitalize()}：{platform_counts[platform]} 次动作（{_pct(platform_counts[platform])}）"
                )
        lines.append(f"- 参与Agent数：{len(participants)}")
        if participants:
            lines.append(f"- 人均动作数：{total_actions / len(participants):.1f}")

        # 2. 动作构成
        lines.append("")
        lines.append("【动作类型构成】")
        for action_type, count in sorted(action_type_counts.items(), key=lambda kv: -kv[1]):
            lines.append(f"- {action_type}：{count} 次（{_pct(count)}）")

        # 3. 立场分布 (Public Stance vs Private Stance)
        lines.append("")
        lines.append("【公开立场分布 (Public Expression, 基于模拟配置)】")
        if not stance_map:
            lines.append("- 无法获取：simulation_config.json 缺失或不含 agent_configs，")
            lines.append("  因此本次无公开立场数据可用。")
        else:
            for stance, count in sorted(stance_counts.items(), key=lambda kv: -kv[1]):
                lines.append(f"- {stance}：{count} 次（{_pct(count)}）")
            unknown = stance_counts.get("unknown", 0)
            if unknown:
                lines.append(
                    f"  注：其中 {unknown} 次动作的Agent未出现在配置中，立场记为 unknown。"
                )
                
        lines.append("")
        lines.append("【私下真实意见分布 (Private Stance, 基于LLM动态生成)】")
        if not private_stance_counts:
            lines.append("- 无数据")
        else:
            total_private = sum(private_stance_counts.values())
            for stance, count in sorted(private_stance_counts.items(), key=lambda kv: -kv[1]):
                lines.append(f"- {stance}：{count} 次（{count/total_private*100:.1f}%）")

        # 3.5 Falsification Gap (Private vs Public)
        if any(v > 0 for v in falsification_gap.values()):
            lines.append("")
            lines.append("【Preference Falsification Gap (私下意见 vs 公开表达)】")
            total_opposing = falsification_gap["silent_dissent"] + falsification_gap["vocal_dissent"]
            if total_opposing > 0:
                silent_pct = (falsification_gap["silent_dissent"] / total_opposing) * 100
                lines.append(f"- 反对派 (Opposing) 中，选择沉默的比例: {silent_pct:.1f}% ({falsification_gap['silent_dissent']}/{total_opposing})")
            
            total_supportive = falsification_gap["silent_support"] + falsification_gap["vocal_support"]
            if total_supportive > 0:
                silent_pct_sup = (falsification_gap["silent_support"] / total_supportive) * 100
                lines.append(f"- 支持派 (Supportive) 中，选择沉默的比例: {silent_pct_sup:.1f}% ({falsification_gap['silent_support']}/{total_supportive})")

        # 4. 时间线
        if round_counts:
            lines.append("")
            lines.append("【时间线（每轮动作数）】")
            for round_num in sorted(round_counts):
                lines.append(f"- 第 {round_num} 轮：{round_counts[round_num]} 次")
            peak_round = max(round_counts, key=lambda r: round_counts[r])
            lines.append(f"- 峰值轮次：第 {peak_round} 轮（{round_counts[peak_round]} 次）")

        # 5. 核心参与者
        lines.append("")
        lines.append("【最活跃Agent（Top 10）】")
        top_agents = sorted(agent_action_counts.items(), key=lambda kv: -kv[1])[:10]
        for agent_id, count in top_agents:
            name = agent_names.get(agent_id, f"Agent_{agent_id}")
            stance = stance_map.get(agent_id, "unknown")
            lines.append(f"- {name}（id={agent_id}, 立场={stance}）：{count} 次动作")

        lines.append("")
        lines.append("【说明】以上数字均为对动作日志的直接计数，可直接引用。")
        lines.append("本工具不提供情感极性（sentiment）数据——动作日志中不含该字段，")
        lines.append("如需情感倾向，请改用 interview_agents 或引用动作原文内容。")
