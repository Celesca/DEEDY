...
# 泰国作息时间配置（基于曼谷时间 UTC+7）
#
# 【唯一事实来源】本模块所有时段相关的默认值都从这里派生，
# 请勿在别处另行硬编码时段列表，否则会出现各处配置互相矛盾的情况。
# 注意：泰国社交媒体活跃到深夜，因此 23 点与 0 点仍算高峰，凌晨1点才进入静默。
THAI_TIMEZONE_CONFIG = {
    # 深夜静默时段
    ...
    # 晚间高峰（含深夜刷手机时段）
    "peak_hours": [18, 19, 20, 21, 22, 23, 0],
    # 活跃度系数
...
class TimeSimulationConfig:
    ...
    # 以下时段默认值全部派生自 THAI_TIMEZONE_CONFIG（唯一事实来源）
    # 使用 list(...) 复制，避免多个实例共享同一个可变列表

    # 高峰时段（18点至次日0点，泰国人最活跃的时间）
    peak_hours: List[int] = field(
        default_factory=lambda: list(THAI_TIMEZONE_CONFIG["peak_hours"])
    )
    peak_activity_multiplier: float = THAI_TIMEZONE_CONFIG["activity_multipliers"]["peak"]

    # 低谷时段（凌晨1-5点，几乎无人活动）
    off_peak_hours: List[int] = field(
        default_factory=lambda: list(THAI_TIMEZONE_CONFIG["dead_hours"])
    )
    off_peak_activity_multiplier: float = THAI_TIMEZONE_CONFIG["activity_multipliers"]["dead"]

    # 早间时段
    morning_hours: List[int] = field(
        default_factory=lambda: list(THAI_TIMEZONE_CONFIG["morning_hours"])
    )
    morning_activity_multiplier: float = THAI_TIMEZONE_CONFIG["activity_multipliers"]["morning"]

    # 工作时段
    work_hours: List[int] = field(
        default_factory=lambda: list(THAI_TIMEZONE_CONFIG["work_hours"])
    )
    work_activity_multiplier: float = THAI_TIMEZONE_CONFIG["activity_multipliers"]["work"]
...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        # ========== 生成可复现的社交拓扑 (Homophily) ==========
        logger.info("构建社交网络拓扑 (Homophily)...")
        # 简单基于立场的Homophily
        import random
        for i, agent in enumerate(all_agent_configs):
            for j, other in enumerate(all_agent_configs):
                if i == j:
                    continue
                # Same stance -> higher probability to follow
                prob = 0.3 if agent.stance == other.stance else 0.05
                # High influence -> more followers
                prob *= other.influence_weight
                if random.random() < prob:
                    agent.following_ids.append(other.agent_id)
...
- peak_hours (int数组): 高峰时段，根据事件参与群体调整（泰国基准：18-23及0点）
- off_peak_hours (int数组): 低谷时段（泰国基准：凌晨1-5点；泰国网民深夜仍活跃，0点不算低谷）
...
- reasoning (string): 简要说明为什么这样配置"""
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        ...
        return {
            ...
            # 全部派生自 THAI_TIMEZONE_CONFIG，避免与 TimeSimulationConfig 不一致
            "peak_hours": list(THAI_TIMEZONE_CONFIG["peak_hours"]),
            "off_peak_hours": list(THAI_TIMEZONE_CONFIG["dead_hours"]),
            "morning_hours": list(THAI_TIMEZONE_CONFIG["morning_hours"]),
            "work_hours": list(THAI_TIMEZONE_CONFIG["work_hours"]),
            "reasoning": "使用默认泰国作息配置（曼谷时间，每轮1小时）"
        }
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        ...
        return TimeSimulationConfig(
            ...
            peak_hours=result.get("peak_hours", list(THAI_TIMEZONE_CONFIG["peak_hours"])),
            off_peak_hours=result.get("off_peak_hours", list(THAI_TIMEZONE_CONFIG["dead_hours"])),
...
## 任务
为每个实体生成活动配置，注意：
- **时间符合目标用户群体作息**：以下为参考（泰国曼谷时间 UTC+7），请根据模拟场景调整
...
- **个人**（Student/Person/Alumni）：活跃度高(0.6-0.9)，主要晚间活动(18-24)，响应快(1-15分钟)，影响力低(0.8-1.2)
