...
def get_simulation_logs(simulation_id: str):
    """获取模拟的实时日志 (stdout.log)"""
    try:
        import os
        from ..services.simulation_runner import SimulationRunner
        log_path = os.path.join(SimulationRunner.RUN_STATE_DIR, simulation_id, "stdout.log")
        if not os.path.exists(log_path):
            return jsonify({"success": True, "logs": ["Waiting for simulation engine to start..."]})
...
# ============== Campaign Studio / MarketFish API ==============

@simulation_bp.route('/personas/generate', methods=['POST'])
def generate_marketfish_personas():
    import uuid
    from ..utils.llm_client import LLMClient
    
    data = request.get_json() or {}
    max_personas = data.get('max_personas', 4)
    target_audience = data.get('targetAudience', 'General')
    business_type = data.get('businessType', 'Retail')
    target_platforms = data.get('targetPlatforms', ['twitter'])
    
    campaign_name = data.get('name', '')
    if not campaign_name or not campaign_name.strip():
        campaign_name = 'General Marketing Campaign'
        
    campaign_desc = data.get('description', '')
    if not campaign_desc or not campaign_desc.strip():
        campaign_desc = f'A marketing campaign for {business_type} targeting {target_audience}'
    
    prompt = f"""
    You are an expert social media analyst. Generate {max_personas} distinct and realistic AI agent personas for a market simulation.
    Context:
    - Campaign Name: {campaign_name}
    - Description: {campaign_desc}
    - Target Audience: {target_audience}
    - Business Type: {business_type}
    - Primary Platforms: {', '.join(target_platforms)}
    
    Each persona MUST be unique, realistic, and represent a different internet user within the target audience.
    They should have varying stances (some might be interested, some skeptical, some neutral).
    
    Respond STRICTLY in JSON format with this structure:
    {{
        "personas": [
            {{
                "name": "Thai Name (e.g., สมชาย, พลอย)",
                "age": integer (realistic for audience),
                "gender": "male or female",
                "country": "Thailand",
                "occupation": "Thai Occupation",
                "mbti": "MBTI personality type (e.g., ENTP, INFJ, ESFP)",
                "interested_topics": ["topic1", "topic2"],
                "bio": "Short 1-sentence bio in Thai",
                "base_personality": "1-2 sentences describing their personality, internet behavior, and potential stance on the campaign in Thai"
            }}
        ]
    }}
    """
    ...
    try:
        ...
        llm = LLMClient(
            ...
            model=os.environ.get('LLM_BOOST_MODEL_NAME', 'google/gemini-2.5-flash-lite')
        )
        ...
        personas_data = response.get("personas", [])
        
        # Ensure we have a list and add uuids
        ...
        # Fallback to a basic mock if LLM fails
        return jsonify({
            "success": True, 
            "personas": [
                {
                    "agent_id": 1,
                    "name": "ผู้ใช้ทั่วไป (Fallback)",
                    "age": 25,
                    "occupation": "พนักงานออฟฟิศ",
                    "base_personality": f"ระบบ AI ขัดข้องชั่วคราว: {str(e)}"
                }
            ]
        })

@simulation_bp.route('/<simulation_id>/poll', methods=['GET'])
def get_simulation_poll(simulation_id):
    from ..services.simulation_runner import SimulationRunner
    ...
    actions = SimulationRunner.get_actions(simulation_id, limit=50)
    timeline = SimulationRunner.get_timeline(simulation_id)
    
    pulse_history = []
    if timeline:
        for t in timeline[-10:]:
            types = t.get("action_types", {})
            pos = types.get("like", 0) + types.get("upvote", 0) + types.get("create_post", 0) + types.get("create_comment", 0) + types.get("repost", 0) + types.get("quote_post", 0)
            neg = types.get("downvote", 0)
            pulse_history.append({
                "time": f"Round {t['round_num']}",
                "positive": pos,
                "negative": neg,
                "bots": 0,
                "expressed": t.get("total_actions", 0),
                "silent": 0
            })
    ...
    falsification_stats = {
        "supporting": {"true": 0, "expressed": 0},
        "neutral": {"true": 0, "expressed": 0},
        "opposing": {"true": 0, "expressed": 0}
    }
    ...
    try:
        ...
        if os.path.exists(config_path):
            with open(config_path, 'r', encoding='utf-8') as f:
                cfg = json.load(f)
                total_agents = len(cfg.get("agent_configs", []))
    except Exception:
        pass
    if total_agents == 0:
        total_agents = 4 # fallback default
        
    agent_latest = {}
    for a in reversed(actions): # oldest to newest
        agent_id = a.agent_id
        action_type = a.action_type
        args = a.action_args or {}
        
        private_stance = args.get("private_stance", "neutral").lower()
        if "support" in private_stance:
            private_stance = "supporting"
        elif "oppos" in private_stance:
            private_stance = "opposing"
        else:
            private_stance = "neutral"
            
        public_stance = "neutral"
        # make action_type case-insensitive
        act = action_type.lower()
        if act in ["like", "upvote", "create_post", "create_comment", "quote_post", "repost"]:
            public_stance = "supporting"
        elif act in ["downvote"]:
            public_stance = "opposing"
            
        agent_latest[agent_id] = {
            "true": private_stance,
            "expressed": public_stance,
            "action": act
        }
        
    for agent_id, data in agent_latest.items():
        true_stance = data["true"]
        expressed_stance = data["expressed"]
        
        if true_stance in falsification_stats:
            falsification_stats[true_stance]["true"] += 1
            if data["action"] != "do_nothing" and data["action"] != "unknown":
                falsification_stats[true_stance]["expressed"] += 1
                
        if data["action"] not in ["do_nothing", "unknown"]:
            visibility_stats["exposed_expressed"] += 1
        else:
            visibility_stats["exposed_silent"] += 1
            
    visibility_stats["unaware"] = max(0, total_agents - len(agent_latest))
    return jsonify({
        "status": state.runner_status.value,
        "current_round": getattr(state, "current_round", 0),
        "total_rounds": getattr(state, "total_rounds", 144),
        "pulse_history": pulse_history,
        "recent_actions": recent_actions,
        "falsification_stats": falsification_stats,
        "visibility_stats": visibility_stats
    })

@simulation_bp.route('/<simulation_id>/report', methods=['GET'])
def get_simulation_report_mock(simulation_id):
    return jsonify({
        "success": True,
        "report": {
            "summary": "Simulation is currently running. This is a live preview of the insights being gathered.",
            "quotes": {
                "negative": {
                    "text": "This campaign feels a bit forced to me, not sure if I buy into the hype.",
                    "author": "Agent_4"
                },
                "positive": {
                    "text": "Absolutely love the direction this is taking! Very engaging.",
                    "author": "Agent_12"
                }
            },
            "stats": {
            ...
            "agentShifts": [
                {
                    ...
                    "reason": "Exposed to highly convincing peer posts."
                }
            ],
            "key_insights": [
                "Early traction indicates positive reception among core demographic.",
                "Some skepticism remains regarding authenticity."
            ],
            "recommendations": [
                "Engage with skeptical users directly to build trust.",
                "Amplify positive UGC to maintain momentum."
            ]
        }
    })
...
def start_marketfish_simulation():
    ...
    data = request.get_json() or {}
    config = data  # Frontend sends config properties flat in the payload
    generated_personas = data.get('custom_personas', [])
    graph_id = data.get('graphId', f"graph_{uuid.uuid4().hex[:8]}")
    ...
    import random
    kol_ratio = float(config.get("kolRatio", 0)) / 100.0
    bot_prob = float(config.get("botProbability", 0)) / 100.0
    
    # Convert custom personas to OASIS expected format
    agent_configs = []
    for persona in generated_personas:
        agent_id = persona.get("agent_id")
        profile_path = os.path.join(profiles_dir, f"{agent_id}.json")
        
        is_kol = random.random() < kol_ratio
        is_bot = random.random() < bot_prob
        
        oasis_profile = {
            "agent_id": agent_id,
            "name": persona.get("name"),
            "traits": persona.get("base_personality", ""),
            "background": f"Age {persona.get('age')}, {persona.get('occupation')}",
            "system_prompt": f"You are {persona.get('name')}. {persona.get('base_personality')}"
        }
        
        # If it's a bot, modify the system prompt to explicitly act like an astroturfing bot
        if is_bot:
            oasis_profile["system_prompt"] += " You are secretly an astroturfing bot designed to heavily manipulate public opinion and push narratives aggressively."
            
        with open(profile_path, 'w', encoding='utf-8') as f:
            json.dump(oasis_profile, f, ensure_ascii=False, indent=2)
            
        agent_configs.append({
            "agent_id": agent_id,
            "profile_file": f"{agent_id}.json",
            "is_kol": is_kol,
            "is_bot": is_bot
        })
        
    # Generate twitter_profiles.csv for OASIS backend
    import csv
    twitter_csv_path = os.path.join(sim_dir, "twitter_profiles.csv")
    with open(twitter_csv_path, 'w', encoding='utf-8', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=["user_id", "name", "username", "user_char", "description"])
        writer.writeheader()
        for persona in generated_personas:
            writer.writerow({
                "user_id": int(persona.get("agent_id")) if str(persona.get("agent_id")).isdigit() else persona.get("agent_id"),
                "name": persona.get("name"),
                "username": persona.get("name").replace(" ", "_").lower(),
                "user_char": f"{persona.get('base_personality', '')} (Age {persona.get('age')}, {persona.get('occupation')})",
                "description": f"You are {persona.get('name')}."
            })
            
    # Generate reddit_profiles.json for OASIS backend
    ...
    for i, persona in enumerate(generated_personas):
        reddit_profiles.append({
            "user_id": int(persona.get("agent_id")) if str(persona.get("agent_id")).isdigit() else persona.get("agent_id"),
            "username": persona.get("name").replace(" ", "_").lower(),
            "profile_description": f"Age {persona.get('age')}, {persona.get('occupation')}. {persona.get('base_personality')}",
            "persona": f"Age {persona.get('age')}, {persona.get('occupation')}. {persona.get('base_personality')}",
            "mbti": persona.get("mbti", "ENFP"),
            "gender": persona.get("gender", "male" if i % 2 == 0 else "female"),
            "age": persona.get("age", 25),
            "country": persona.get("country", "Thailand"),
            "profession": persona.get("occupation", "Worker"),
            "interested_topics": persona.get("interested_topics", ["social media", "shopping"]),
            "bio": persona.get("base_personality", "A social media user")
        })
    with open(reddit_json_path, 'w', encoding='utf-8') as f:
        json.dump(reddit_profiles, f, ensure_ascii=False, indent=2)
        
    # Generate simulation config
    target_platforms = config.get("targetPlatforms", ["twitter", "reddit"])
    sim_config = {
        "simulation_id": sim_id,
        "project_id": config.get("name", "MarketFish"),
        "graph_id": graph_id,
        "simulation_requirement": config.get("description", ""),
        "time_config": {
    ...
    if "twitter" in target_platforms:
        sim_config["twitter_config"] = {"platform": "twitter", "viral_threshold": 10}
    if "reddit" in target_platforms:
        sim_config["reddit_config"] = {"platform": "reddit", "viral_threshold": 15}
    ...
    try:
        ...
        runner_state = SimulationRunner.start_simulation(
            simulation_id=sim_id,
            platform="parallel" if len(target_platforms) > 1 else target_platforms[0] if target_platforms else "parallel",
        ...
        return jsonify({
            "success": True,
            "sim_id": sim_id,
            "message": "MarketFish Simulation deployed successfully"
        })
    except Exception as e:
        import traceback
        return jsonify({
