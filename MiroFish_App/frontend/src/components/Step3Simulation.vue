  ...
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      <div class="status-group">
        <!-- Thai Society 平台进度 -->
        <div class="platform-status thai_society" :class="{ active: runStatus.thai_society_running, completed: runStatus.thai_society_completed }">
          <div class="platform-header">
            <svg class="platform-icon" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path>
            </svg>
            <span class="platform-name">Thai Society</span>
            <span v-if="runStatus.thai_society_completed" class="status-badge">
          ...
          <div class="platform-stats">
            <span class="stat">
              ...
              <span class="stat-value mono">{{ runStatus.thai_society_current_round || 0 }}<span class="stat-total">/{{ runStatus.total_rounds || maxRounds || '-' }}</span></span>
            </span>
            <span class="stat">
              <span class="stat-label">TIME</span>
              <span class="stat-value mono">{{ thaiSocietyElapsedTime }}</span>
            </span>
            <span class="stat">
              <span class="stat-label">ACTS</span>
              <span class="stat-value mono">{{ thaiSocietyActionsCount || 0 }}</span>
            </span>
          </div>
          <!-- Falsification Gap -->
          <div class="falsification-gap" v-if="falsificationGap.total > 0">
            <div class="gap-title">Preference Falsification Gap (Private vs Public)</div>
            <div class="gap-bars">
              <div class="gap-bar-group">
                <div class="gap-label">Opposing (Silent {{ Math.round(falsificationGap.silent_dissent / max(1, falsificationGap.total_dissent) * 100) }}%)</div>
                <div class="gap-progress">
                  <div class="gap-fill silent" :style="{ width: `${(falsificationGap.silent_dissent / max(1, falsificationGap.total_dissent)) * 100}%` }"></div>
                  <div class="gap-fill vocal" :style="{ width: `${(falsificationGap.vocal_dissent / max(1, falsificationGap.total_dissent)) * 100}%` }"></div>
                </div>
              </div>
              <div class="gap-bar-group">
                <div class="gap-label">Supportive (Silent {{ Math.round(falsificationGap.silent_support / max(1, falsificationGap.total_support) * 100) }}%)</div>
                <div class="gap-progress">
                  <div class="gap-fill silent" :style="{ width: `${(falsificationGap.silent_support / max(1, falsificationGap.total_support)) * 100}%` }"></div>
                  <div class="gap-fill vocal" :style="{ width: `${(falsificationGap.vocal_support / max(1, falsificationGap.total_support)) * 100}%` }"></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="timeline-header" v-if="allActions.length > 0">
        <div class="timeline-stats">
          ...
          <span class="platform-breakdown">
            <span class="breakdown-item thai_society">
              <svg class="mini-icon" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path>
              </svg>
              <span class="mono">{{ thaiSocietyActionsCount }}</span>
            </span>
          </span>
        </div>
      </div>
...
// Timeline feed filters out NO_ACTION, but gap calculation uses them
const chronologicalActions = computed(() => {
  return allActions.value.filter(a => a.action_type !== 'NO_ACTION')
})

// Các平台动作计数
const thaiSocietyActionsCount = computed(() => {
  return allActions.value.filter(a => a.platform === 'thai_society' && a.action_type !== 'NO_ACTION').length
})

// Falsification Gap Calculation
const falsificationGap = computed(() => {
  const gap = {
    silent_dissent: 0,
    vocal_dissent: 0,
    silent_support: 0,
  ...
  allActions.value.forEach(a => {
    if (!a.action_args) return
    const stance = a.action_args.private_stance
    if (stance === 'opposing') {
      if (a.action_type === 'SILENT_SHIFT' || a.action_type === 'NO_ACTION') gap.silent_dissent++
      else gap.vocal_dissent++
      gap.total++
    } else if (stance === 'supportive') {
      if (a.action_type === 'SILENT_SHIFT' || a.action_type === 'NO_ACTION') gap.silent_support++
      else gap.vocal_support++
      gap.total++
    }
  })
  
  gap.total_dissent = gap.silent_dissent + gap.vocal_dissent
  gap.total_support = gap.silent_support + gap.vocal_support
  return gap
})
...
// Thai Society平台的模拟流逝时间
const thaiSocietyElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.thai_society_current_round || 0)
})
...
const prevThaiSocietyRound = ref(0)
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 检测轮次变化并输出日志
      if (data.thai_society_current_round > prevThaiSocietyRound.value) {
        addLog(`[Thai Society] R${data.thai_society_current_round}/${data.total_rounds} | A:${data.thai_society_actions_count}`)
        prevThaiSocietyRound.value = data.thai_society_current_round
      }
...
const checkPlatformsCompleted = (data) => {
  ...
  const tsCompleted = data.thai_society_completed === true
  const tsEnabled = (data.thai_society_actions_count > 0) || data.thai_society_running || tsCompleted
  
  if (!tsEnabled) return false
  if (tsEnabled && !tsCompleted) return false
...
/* Thai Society Styles */
.platform-status.thai_society .platform-icon { color: #1da1f2; }
.breakdown-item.thai_society { color: #1da1f2; }
.timeline-item.thai_society .marker-dot { background: #1da1f2; }
.timeline-item.thai_society .timeline-marker { border-color: rgba(29, 161, 242, 0.2); }

/* Falsification Gap Chart */
.falsification-gap {
  margin-top: 16px;
  padding-top: 12px;
...
.gap-fill.silent {
  background-color: #cbd5e0; /* Gray for silent */
}
.gap-fill.vocal {
  background-color: #1da1f2; /* Blue for vocal */
}
</style>
