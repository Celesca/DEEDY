...
const sendToAgent = async (message) => {
  ...
  if (res.success && res.data) {
    ...
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // 优先寻找 thai_society 平台
      const tsKey = `thai_society_${agentId}`
      const redditKey = `reddit_${agentId}`
      const twitterKey = `twitter_${agentId}`
      const agentResult = resultsDict[tsKey] || resultsDict[redditKey] || resultsDict[twitterKey] || Object.values(resultsDict)[0]
      if (agentResult) {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      for (const interview of interviews) {
        ...
        if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
          const tsKey = `thai_society_${agentIdx}`
          const redditKey = `reddit_${agentIdx}`
          const twitterKey = `twitter_${agentIdx}`
          
          // Try thai_society, then reddit, then twitter, then any key that ends with _agentIdx
          const agentResult = resultsDict[tsKey] || resultsDict[redditKey] || resultsDict[twitterKey] || 
                              Object.entries(resultsDict).find(([k, v]) => k.endsWith(`_${agentIdx}`))?.[1]
