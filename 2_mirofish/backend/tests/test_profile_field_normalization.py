...
def test_normalized_profile_serializes_to_twitter_and_reddit(tmp_path):
    ...
    # user_char = bio + persona，后面追加人口统计信息。
    # OASIS 的 Twitter CSV 只有五列，user_char 是 age/gender/region/profession
    # 等字段唯一能进入LLM系统提示的通道，因此这里必须携带这些信息。
    assert twitter["user_char"].startswith("公开简介 详细, 人设")
    assert "MBTI ENFP" in twitter["user_char"]
