...
class OasisAgentProfile:
    ...
    region: Optional[str] = None  # 泰国地区（曼谷/中部/北部/东北/南部）
    ...
    def __post_init__(self):
        ...
        self.region = _coerce_to_str(self.region) or None
...
class OasisProfileGenerator:
    ...
    # Thai demographic distributions
    THAI_REGIONS = [
        "Bangkok", "Central", "North", "Northeast", "South"
    ]
    
    # Base online survey demographics for Thai
    THAI_PROFESSIONS = [
        "Student", "Company Employee", "Government Official", 
        "Freelancer", "Business Owner", "Unemployed"
    ]
...
- 机构账号发言要符合其身份定位"""
    ...
    def _generate_profile_rule_based(
        ...
        if entity_type_lower in ["student", "alumni"]:
            ...
            return {
                ...
                "mbti": random.choice(self.MBTI_TYPES),
                "country": "Thailand",
                "region": random.choice(self.THAI_REGIONS),
                "profession": entity_type,
                "interested_topics": ["General", "Social Issues", "News", "Entertainment"],
            }
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            target_total_agents: DEEDY requested population size
        ...
        synthetic_count = max(0, target_total_agents - core_count)
        total = core_count + synthetic_count
            ...
            with lock:
                ...
                try:
                    if output_platform == "reddit":
                        ...
                        if profiles_data:
                            # to_twitter_format() 按需输出可选字段（age/region/mbti等），
                            # 各profile的键集合可能不同。若只取第一条的键作表头，
                            # 后面出现新键时 DictWriter 会抛错并被外层静默吞掉，
                            # 导致profile文件写不出来。这里取所有profile键的并集。
                            fieldnames = []
                            seen = set()
        ...
        logger.info(f"开始并行生成 {total} 个Agent人设（Core: {core_count}, Synthetic: {synthetic_count}）...")
        print(f"\n{'='*60}")
        print(f"开始生成Agent人设 - 共 {total} 个实体，并行数: {parallel_count}")
        print(f"{'='*60}\n")
        
        def _generate_synthetic_public_agent(idx: int) -> tuple:
            set_locale(current_locale)
            age = random.choices(
                [random.randint(18, 24), random.randint(25, 34), random.randint(35, 44), random.randint(45, 54), random.randint(55, 65)],
                weights=[0.25, 0.35, 0.20, 0.15, 0.05]
            )[0]
            region = random.choice(self.THAI_REGIONS)
            profession = random.choice(self.THAI_PROFESSIONS)
            profile = OasisAgentProfile(
                user_id=idx,
                # 带上 idx 保证用户名唯一（随机后缀只有900种，几十个Agent就会撞名）
                user_name=self._generate_username(f"thai_user_{idx}"),
                name=f"Thai User {idx}",
                bio=f"{profession} from {region}, Thailand",
                persona=(
                    f"A regular Thai netizen, age {age}, working as a {profession.lower()} "
                    f"in the {region} region. Follows news and trends on social media, "
                    f"and reacts as an ordinary member of the Thai public rather than a public figure."
                ),
                source_entity_uuid=f"synthetic_{idx}",
                source_entity_type="public_agent",
                age=age,
                gender=random.choice(["male", "female"]),
                mbti=random.choice(self.MBTI_TYPES),
                country="Thailand",
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for idx, entity in enumerate(entities):
                future_to_entity[executor.submit(generate_single_profile, idx, entity)] = (idx, entity.name, entity.get_entity_type() or "Entity")
                
            # Submit synthetic agents
            for i in range(synthetic_count):
                idx = core_count + i
                future_to_entity[executor.submit(_generate_synthetic_public_agent, idx)] = (idx, f"Thai User {idx}", "public_agent")
            
            # 收集结果
            for future in concurrent.futures.as_completed(future_to_entity):
                idx, entity_name, entity_type = future_to_entity[future]
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                ...
                # OASIS 的 Twitter CSV 只有 user_id/name/username/user_char/description
                # 五列，age/gender/region/profession 等结构化字段全部到不了模拟里。
                # user_char 是唯一会进入LLM系统提示的通道，因此把人口统计信息
                # 拼进文本，否则泰国本地化的画像设定在Twitter模拟中完全失效。
                demographics = []
                if profile.age:
                    demographics.append(f"age {profile.age}")
                if profile.gender:
                    demographics.append(profile.gender)
                if profile.profession:
                    demographics.append(profile.profession)
                if profile.region:
                    demographics.append(f"from {profile.region}")
                if profile.country:
                    demographics.append(profile.country)
                if profile.mbti:
                    demographics.append(f"MBTI {profile.mbti}")
                if demographics:
                    user_char = f"{user_char} ({', '.join(demographics)})"
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                ...
                # 本项目模拟的是泰国社会，缺省国别必须是 Thailand（原为"中国"）
                "country": profile.country if profile.country else "Thailand",
            }
