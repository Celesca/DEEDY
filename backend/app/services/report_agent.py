...
3. 立场分布：按动作数统计的 supportive / opposing / neutral / observer 占比
   （立场取自 simulation_config.json，为配置阶段确定的值）
4. 时间线：每一轮的动作数及峰值轮次
5. 最活跃Agent Top 10（含其立场）

【使用场景】
- 报告中需要引用确凿的宏观数据
- 描述传播规模、参与度、立场对比、爆发时点
...
class ReportAgent:
    ...
    def _calculate_trace_statistics(self) -> str:
        ...
        # 3. 立场分布
        lines.append("")
        lines.append("【立场分布（按动作数计）】")
        if not stance_map:
            lines.append("- 无法获取：simulation_config.json 缺失或不含 agent_configs，")
            lines.append("  因此本次无立场数据可用，请勿在报告中推测立场比例。")
        else:
